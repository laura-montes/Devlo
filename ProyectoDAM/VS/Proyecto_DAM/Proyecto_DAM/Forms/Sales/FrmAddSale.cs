...
namespace Proyecto_DAM.Forms.Sales
{
	public partial class FrmAddSale : Form
	{
		...
		private void BtnSave_Click(object sender, EventArgs e)
		{
			...
			decimal totalConIva = decimal.Parse(price.ToString("0.00")) + Math.Round(Convert.ToDecimal(decimal.Parse(price.ToString("0.00")) * 0.21M), 2);

			utilities.pa_add_sale(int.Parse(CmbBxContact.SelectedValue.ToString()), totalConIva, null, DatePicker.Value.Date, lineas);
